...
class HookService {
  ...
  public async hasHooks(tableName: string, hookName: string) {
    const globalHooks = this.hooks.get("_global");
    const tableHooks = this.hooks.get(tableName);
    if (globalHooks && typeof globalHooks[hookName] === "function") return true;
    if (tableHooks && typeof tableHooks[hookName] === "function") return true;
    else return false;
  }
